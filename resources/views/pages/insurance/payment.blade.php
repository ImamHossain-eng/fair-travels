    ...
    <div class="container">
        <div class="card mt-4">
            <div class="card-header bg-warning">
                <div class="card-title">
                    <h3 class="text-center text-dark">
                        Payment for Insurance Service from {{$insurance->starting_date->format('F d, Y')}} to {{$insurance->ending_date->format('F d, Y')}}
                    </h3>
                </div>
            </div>
            <div class="card-body" style="background: #EBF8A4;">
                ...
                <form action="{{route('insurance.payment.store', $insurance->id)}}" method="POST">
                    @csrf 
                    ...
                    <div class="form-group mb-4">
                        <input type="hidden" step="any" name="amount" class="form-control" placeholder="Enter Amount" value="{{$insurance->amount}}">
                        
                        <input type="number" step="any" name="amountView" class="form-control" placeholder="Enter Amount" value="{{$insurance->amount}}" disabled>
                    </div>
                    <div class="form-group mb-4">
